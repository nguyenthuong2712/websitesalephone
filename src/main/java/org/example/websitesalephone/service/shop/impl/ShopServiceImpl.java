...
public class ShopServiceImpl implements ShopService {
    ...
    public CommonResponse getActiveShops() {
        List<ShopRegistration> shops = shopRegistrationRepository.findAll();
        return CommonResponse.builder()
                .code(CommonResponse.CODE_SUCCESS)
                .data(shops.stream()
                        .filter(s -> s.getStatus() == ShopRegistrationStatus.APPROVED || s.getStatus() == ShopRegistrationStatus.PENDING)
                        .map(s -> Map.of("id", s.getId(), "name", s.getShopName()))
                        .toList())
                .build();
    }
}
