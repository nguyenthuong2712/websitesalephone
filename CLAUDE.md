...
- `config/SecurityConfig.java` installs the JWT filter and currently permits all `/api/auth`, `/api/product`, `/api/cart`, `/api/dynamic`, `/api/user`, `/api/product-images`, `/api/pdf`, and `/api/order` routes at the HTTP layer, so many service methods rely on the authenticated principal being present rather than URL-level access control.
