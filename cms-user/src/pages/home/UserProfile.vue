...
<template>
  ...
  <article class="profile-card" aria-label="Thông tin người dùng">
    <aside class="profile-left">
      ...
      <div class="name" id="displayName">{{ user.fullName }}</div>
      <div class="role">{{ user.role === 'ADMIN' ? 'Quản Lý' : user.role === 'STAFF' ? 'Nhân Viên' : 'Khách hàng' }}</div>
      <div class="contact-quick" aria-hidden="false">
        <div class="chip" id="quickEmail">{{ user.email }}</div>
        <div class="chip" id="quickPhone">{{ user.telNo }}</div>
      </div>
    ...
    <section class="profile-right">
      ...
      <div class="field-list" v-if="!isEditing">
        <div class="field">
          ...
          <div class="value" id="v-fullname">{{ user.fullName }}</div>
        </div>

        <div class="field">
          <div class="label">Email</div>
          <div class="value" id="v-email">{{ user.email }}</div>
        </div>

        <div class="field">
          <div class="label">Địa chỉ</div>
          <div class="value" id="v-address">{{ user.address }}</div>
        </div>

        <div class="field">
          <div class="label">Số điện thoại</div>
          <div class="value" id="v-phone">{{ user.telNo }}</div>
        </div>

        <div class="field" role="group" aria-label="Giới tính">
          <div class="label">Giới tính</div>
          <div class="value" id="v-gender">{{ user.gender === 'male' ? 'Nam' : user.gender === 'female' ? 'Nữ' : 'Khác' }}</div>
        </div>
      </div>
      ...
      <form class="edit-form" v-if="isEditing">
        ...
        <div class="form-row">
          ...
          <input v-model="user.telNo" type="tel" />
        </div>

        <div class="form-row">
          <label class="label" for="genderSelect">Giới tính</label>
          <select v-model="user.gender" aria-label="Chọn giới tính">
